// "Métiers & compétences" tab (VDAB Competent). Wrapped in an IIFE so its
// internals don't leak into the other tab's script — app-shell.js drives it
// through the small surface returned at the bottom.
const CompetentApp = (function () {
  ...
  function occChip(opCode) {
    ...
    return `<button class="chip" data-nav="#/${PREFIX}/metier/${esc(opCode)}"><span class="code">${esc(opCode)}</span>${esc(label)}</button>`;
  }
  ...
  function skillChip(skillCode, cls) {
    ...
    return `<button class="chip ${cls || ''}" data-nav="#/${PREFIX}/competence/skill/${esc(skillCode)}"><span class="code">${esc(skillCode)}</span>${esc(label)}</button>`;
  }

  function softChip(code) {
    const s = DATA.softSkills[code];
    const label = s ? (pick(s.title, currentLang) || code) : code;
    return `<button class="chip soft" data-nav="#/${PREFIX}/competence/soft/${esc(code)}"><span class="code">${esc(code)}</span>${esc(label)}</button>`;
  }

  function digitalChip(code) {
    const s = DATA.digitalSkills[code];
    const label = s ? (pick(s.title, currentLang) || code) : code;
    return `<button class="chip digital" data-nav="#/${PREFIX}/competence/digital/${esc(code)}"><span class="code">${esc(code)}</span>${esc(label)}</button>`;
  }

  function sectorChip(code) {
    const s = DATA.econSectors[code];
    const label = s ? (pick(s.title, currentLang) || code) : code;
    return `<button class="chip" data-nav="#/${PREFIX}/secteur/${esc(code)}"><span class="code">${esc(code)}</span>${esc(label)}</button>`;
  }

  function areaChip(code) {
    const s = DATA.areasOfInterest[code];
    const label = s ? (pick(s.title, currentLang) || code) : code;
    return `<button class="chip" data-nav="#/${PREFIX}/interet/${esc(code)}"><span class="code">${esc(code)}</span>${esc(label)}</button>`;
  }
  ...
  function renderOccResults(container, query) {
    if (!query) {
      const all = Object.values(DATA.occupations)
        .map((op) => ({ nav: `#/${PREFIX}/metier/${op.id}`, code: op.id, label: pick(op.title, currentLang) || op.id, sub: null, kindLabel: null, kindCls: '' }))
  ...
  function renderOccupationPage(opCode) {
    ...
    const wrap = el(`
      <div>
        <div class="breadcrumb"><a data-nav="#/${PREFIX}">Accueil</a> / Métier</div>
  ...
  function renderLinkedEntityPage(item, kindLabel) {
    ...
    const wrap = el(`
      <div>
        <div class="breadcrumb"><a data-nav="#/${PREFIX}">Accueil</a> / ${esc(kindLabel)}</div>
        <div class="detail-header">
